...
class Profile(models.Model):
    ...
    # Use strings for Decimal defaults to maintain precision
...
class Order(models.Model):
    ...
    commission_rate = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('1.00'))
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    created_at = models.DateTimeField(auto_now_add=True)
    scheduled_at = models.IntegerField(null=True, blank=True)

    def save(self, *args, **kwargs):
        # Ensure we are working with Decimals for the calculation
    ...
    def __str__(self):
        username = self.user.username if self.user else 'Template'
        return f"{self.product_name} ({username})"

# CLEANER SIGNAL LOGIC
@receiver(post_save, sender=User)
def manage_user_profile(sender, instance, created, **kwargs):
    if created:
        Profile.objects.create(user=instance)
    else:
        # Check if profile exists before trying to save it to prevent errors
